  ...
  if (photos.length === 0) {
    return (
      <div className="mx-auto max-w-md rounded-2xl border border-dashed border-white/15 bg-white/5 p-10 text-center">
        ...
        <p className="mt-4 font-semibold">New photos are on the way</p>
        <p className="mt-1 text-sm text-muted-foreground">
          This collection is being curated right now.
        </p>
        {emptyLink && (
          <a
            href={emptyLink}
          ...
          >
            ...
            {emptyLinkLabel ?? "Follow on Instagram"}
          </a>
        )}
      </div>
    );
...
function Tile({
  ...
  return (
    <motion.button
      ...
      className={cn(
        "gallery-item group relative block w-full cursor-zoom-in break-inside-avoid",
        "focus-visible:outline focus-visible:outline-2 focus-visible:outline-primary"
      )}
      aria-label={`Open photo ${index + 1}`}
    >
      {children}
      <span className="absolute inset-0 flex items-center justify-center rounded-xl bg-black/0 transition-colors duration-300 group-hover:bg-black/35">
